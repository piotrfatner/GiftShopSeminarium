...
public class AuthenticationServiceImplTest {
    ...
    public void findByPasswordResetCode() {
        ...
        authenticationService.findByPasswordResetCode(TestConstants.USER_PASSWORD_RESET_CODE);

        when(userRepository.findByPasswordResetCode(TestConstants.USER_PASSWORD_RESET_CODE)).thenReturn(user);
        assertEquals(TestConstants.USER_PASSWORD_RESET_CODE, user.getPasswordResetCode());
        verify(userRepository, times(1)).findByPasswordResetCode(TestConstants.USER_PASSWORD_RESET_CODE);
    }
    ...
    public void login() {
        ...
        when(userRepository.findByEmail(TestConstants.USER_EMAIL)).thenReturn(user);
        assertEquals(123L, user.getId());
        assertEquals(TestConstants.USER_EMAIL, user.getEmail());
        assertEquals(TestConstants.FIRST_NAME, user.getFirstName());
        authenticationService.login(TestConstants.USER_EMAIL);
    ...
    public void activateUser() {
        ...
        when(userRepository.findByActivationCode(TestConstants.USER_ACTIVATION_CODE)).thenReturn(user);
        when(userRepository.save(user)).thenReturn(user);
