...
const Cart: FC = () => {
    ...
    const deleteFromCart = (perfumeId: number): void => {
        productInCart.delete(perfumeId);

        if (productInCart.size === 0) {
            localStorage.removeItem("perfumes");
            setProductInCart(new Map());
        } else {
            localStorage.setItem("perfumes", JSON.stringify(Array.from(productInCart.entries())));
        }
    ...
    const handleInputChange = (event: ChangeEvent<HTMLInputElement>, perfumeId: number): void => {
        if (isNaN(parseInt(event.target.value)) || parseInt(event.target.value) === 0 || parseInt(event.target.value) > 99) {
            setProductInCart(productInCart.set(perfumeId, 1));
            localStorage.setItem("perfumes", JSON.stringify(Array.from(productInCart.entries())));
        } else {
            setProductInCart(productInCart.set(perfumeId, parseInt(event.target.value)));
            localStorage.setItem("perfumes", JSON.stringify(Array.from(productInCart.entries())));
        }
    ...
    const onIncrease = (perfumeId: number): void => {
        setProductInCart(productInCart.set(perfumeId, productInCart.get(perfumeId) + 1));
        localStorage.setItem("perfumes", JSON.stringify(Array.from(productInCart.entries())));
        dispatch(calculateCartPrice(perfumes));
    };
                ...
                <div>
                    {perfumes.length === 0 ?
                        ...
                        <div>
                            ...
                            {perfumes.map((perfume: ProductClass) => {
                                return (
                                        ...
                                        <div className="row no-gutters">
                                            ...
                                            <div className="col-6">
                                                <div className="card-body">
                                                    <h4 className="card-title">{perfume.product_name}</h4>
                                                    <p className="card-text"><span>{perfume.volume}</span> ml.</p>
                                                </div>
                                            </div>
                                            <div className="col-1 mt-3">
                                                <button className="btn btn-default"
                                                        disabled={productInCart.get(perfume.id) === 99}
                                                ...
                                                <input type="text"
                                                       ...
                                                       style={{width: "45px"}}
                                                       value={productInCart.get(perfume.id)}
                                                       onChange={(event) => handleInputChange(event, perfume.id)}/>
                                                <button className="btn btn-default"
                                                        disabled={productInCart.get(perfume.id) === 1}
                                            ...
                                            <div className="col-2">
                                                <div className="card-body">
                                                    <h5 className="card-title">
                                                        <span>$ {perfume.price * productInCart.get(perfume.id)}</span>
                                                    </h5>
